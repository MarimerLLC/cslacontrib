...
namespace MyCsla
{
    ...
    IComparable, IConvertible, IFormattable, Csla.Serialization.Mobile.IMobileObject
  {
    ...
    {
      _emptyValue =  MyCsla.SmartDate.EmptyValue.MinDate;
      _format = null;
      _initialized = false;
      _date = DateTime.MinValue;
      Date = value;
    ...
    private static string NormalizeInput(string value)
    {
      ...
      string pattern = System.Globalization.DateTimeFormatInfo.CurrentInfo.ShortDatePattern;

      // Normalize separator
      if (Regex.IsMatch(value, @"^\d{1,4}[ .,/\-\\]\d{1,4}[ .,/\-\\]\d{1,4}$", RegexOptions.Compiled) ||
          Regex.IsMatch(value, @"^\d{1,2}[ .,/\-\\]\d{1,2}$", RegexOptions.Compiled))
        value = Regex.Replace(value, @"[ .,/\-\\]", separator, RegexOptions.Compiled);

      if (Regex.IsMatch(value, @"^\d{1,2}" + separator + @"\d{1,2}$", RegexOptions.Compiled))
      {
        // Try to parse day and month of this year
        string[] patternElements = pattern.Split(separator.ToCharArray());
        string[] valueParts = value.Split(separator.ToCharArray());
        value = string.Empty;
        int valueCount = 0;

        for (int patternCount = 0; patternCount < patternElements.Length; patternCount++)
        {
          if (patternElements[patternCount].IndexOf("d") != -1)
          {
            value += valueParts[valueCount] + separator;
            valueCount++;
          }
          else if (patternElements[patternCount].IndexOf("M") > -1)
          {
            value += valueParts[valueCount] + separator;
            valueCount++;
          }
          else if (patternElements[patternCount].IndexOf("y") > -1)
            value += DateTime.Now.Year + separator;
        }
        value = value.Substring(0, value.Length - 1);
      }
      else if (Regex.IsMatch(value, @"^\d{4}$", RegexOptions.Compiled))
      {
        // Try to parse digits-only as day and month of this year
        value = value.Substring(0, 2) + separator + value.Substring(2, 2);
        if (pattern.IndexOf(separator + "y") > -1)
          value = value.Insert(pattern.IndexOf(separator + "y"), separator + DateTime.Now.Year);
        else
          value = value.Insert(pattern.IndexOf("yyyy" + separator), DateTime.Now.Year + separator);
      }
      else if (Regex.IsMatch(value, @"^\d{8}$", RegexOptions.Compiled))
      {
        // Try to parse digits-only as day, month and year
        value = value.Insert(pattern.IndexOf(separator), separator);
        value = value.Insert(pattern.LastIndexOf(separator), separator);
      }

      return value;
    }

    private static bool TryParseExtended(string value, out DateTime result)
    {
        DateTime tmp;
        result = DateTime.MaxValue;
