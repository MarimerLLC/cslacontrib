...
{
	/// <summary>
	///	CategoryProduct Generated by MyGeneration using the CSLA Object Mapping template
	/// </summary>
	[Serializable()]
	[TypeConverter(typeof(CategoryProductConverter))]
	public partial class CategoryProduct : BusinessBase<CategoryProduct>, IVEHasBrokenRules
	{
		...
		// TODO: Check CategoryProduct.GetIdValue to assure that the ID returned is unique
		/// <summary>
		/// Overrides Base GetIdValue - Used internally by CSLA to determine equality
		/// </summary>
		/// <returns>A Unique ID for the current CategoryProduct</returns>
		protected override object GetIdValue()
		{
			return _ProductID;
		}
		// TODO: Replace base CategoryProduct.ToString function as necessary
		/// <summary>
		/// Overrides Base ToString
		/// </summary>
		/// <returns>A string representation of current CategoryProduct</returns>
		...
		internal static CategoryProduct New(string productName)
		{
			return new CategoryProduct(productName);
		}
		internal static CategoryProduct Get(SafeDataReader dr)
		{
			return new CategoryProduct(dr);
		}
		public CategoryProduct()
		{
			MarkAsChild();
			_ProductID = Product.NextProductID;
			_UnitPrice = _CategoryProductExtension.DefaultUnitPrice;
			_UnitsInStock = _CategoryProductExtension.DefaultUnitsInStock;
			_UnitsOnOrder = _CategoryProductExtension.DefaultUnitsOnOrder;
			_ReorderLevel = _CategoryProductExtension.DefaultReorderLevel;
			_Discontinued = _CategoryProductExtension.DefaultDiscontinued;
			ValidationRules.CheckRules();
		}
		private CategoryProduct(string productName)
		{
			MarkAsChild();
		...
		internal CategoryProduct(SafeDataReader dr)
		{
			MarkAsChild();
			Fetch(dr);
		}
		...
		internal void Insert(Category myCategory)
		{
			// if we're not dirty then don't update the database
			if (!this.IsDirty) return;
			SqlConnection cn = (SqlConnection)ApplicationContext.LocalContext["cn"];
			Product.Add(cn, ref _ProductID, _ProductName, _MySupplier, myCategory, _QuantityPerUnit, _UnitPrice, _UnitsInStock, _UnitsOnOrder, _ReorderLevel, _Discontinued);
			MarkOld();
		}
		internal void Update(Category myCategory)
		{
			// if we're not dirty then don't update the database
			if (!this.IsDirty) return;
			SqlConnection cn = (SqlConnection)ApplicationContext.LocalContext["cn"];
			Product.Update(cn, ref _ProductID, _ProductName, _MySupplier, myCategory, _QuantityPerUnit, _UnitPrice, _UnitsInStock, _UnitsOnOrder, _ReorderLevel, _Discontinued);
			MarkOld();
		}
		internal void DeleteSelf(Category myCategory)
		{
			// if we're not dirty then don't update the database
			if (!this.IsDirty) return;
		...
		#region extension
		CategoryProductExtension _CategoryProductExtension = new CategoryProductExtension();
		[Serializable()]
		partial class CategoryProductExtension : extensionBase
		{
		}
		[Serializable()]
		class extensionBase
	...
	#region Converter
	internal class CategoryProductConverter : ExpandableObjectConverter
	{
		public override object ConvertTo(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value, Type destType)
		{
			if (destType == typeof(string) && value is CategoryProduct)
			{
				// Return the ToString value
				return ((CategoryProduct)value).ToString();
			}
...
//// The following is a sample Extension File.  You can use it to create CategoryProductExt.cs
//using System;
