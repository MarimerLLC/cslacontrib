...
namespace CslaContrib.MEF.Server
{
  ...
  /// </code>
  /// </example>
  public class MefFactoryLoader : IObjectFactoryLoader
  {
    /// <summary>
    ...
    public Type GetFactoryType(string factoryName)
    {
      ...
      var part1 = Enumerable.FirstOrDefault(from part in Ioc.Container.Catalog.Parts
                                            where part.ExportDefinitions.Any(item => item.ContractName == typename)
                                            select part);

      if (part1 == null) return null;

      // TODO: should be able to optimize code here 
      var part2 = part1.CreatePart();
      object obj = part2.GetExportedValue(part2.ExportDefinitions.First(item => item.ContractName == typename));
      return obj.GetType();
    }
  }
}
