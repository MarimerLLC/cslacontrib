//-----------------------------------------------------------------------
// <copyright file="CslaActionExtenderMenuBar.cs" company="Marimer LLC">
...
{
  /// <summary>
  /// Extender control (for MenuItem only) providing automation around
  /// data binding to CSLA .NET business objects.
  /// </summary>
  [ToolboxItem(true)]
  [ProvideProperty("ActionType", typeof (MenuItem))]
  [ProvideProperty("PostSaveAction", typeof (MenuItem))]
  [ProvideProperty("RebindAfterSave", typeof (MenuItem))]
  [ProvideProperty("DisableWhenClean", typeof (MenuItem))]
  [ProvideProperty("DisableWhenUseless", typeof (MenuItem))]
  [ProvideProperty("CommandName", typeof (MenuItem))]
  public class CslaActionExtenderMenuBar : System.ComponentModel.Component, IExtenderProvider
  {
    #region Constructors
    ...
    public CslaActionExtenderMenuBar(IContainer container)
    {
    ...
    private Dictionary<MenuItem, CslaActionExtenderProperties> _sources =
      new Dictionary<MenuItem, CslaActionExtenderProperties>();
    ...
    /// </summary>
    /// <param name="ctl">Reference to MenuItem.</param>
    /// <returns></returns>
    [Category("Csla")]
    [Description("Gets or sets the action type for this button.")]
    [Bindable(true)]
    [DefaultValue(CslaFormAction.None)]
    public CslaFormAction GetActionType(MenuItem ctl)
    {
    ...
    public PostSaveActionType GetPostSaveAction(MenuItem ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    [Bindable(true)]
    [DefaultValue(true)]
    public bool GetRebindAfterSave(MenuItem ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    [Browsable(false)]
    public bool GetDisableWhenClean(MenuItem ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    [Obsolete("Use instead DisableWhenUseless")]
    [Browsable(false)]
    public void SetDisableWhenClean(MenuItem ctl, bool value)
    {
      if (_sources.ContainsKey(ctl))
    ...
    [Bindable(true)]
    [DefaultValue(false)]
    public bool GetDisableWhenUseless(MenuItem ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    [Bindable(true)]
    [DefaultValue("")]
    public string GetCommandName(MenuItem ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    /// <summary>
    /// Event indicating the user is clicking on the MenuItem.
    /// </summary>
    [Category("Csla")]
    ...
    /// <summary>
    /// Event indicating the user clicked on the MenuItem.
    /// </summary>
    [Category("Csla")]
    ...
    private void InitializeControls(bool initialEnabling)
    {
      ...
      List<MenuItem> extendedControls = new List<MenuItem>();
      foreach (KeyValuePair<MenuItem, CslaActionExtenderProperties> pair in _sources)
      {
        if (pair.Value.ActionType != CslaFormAction.None)
        {
          MenuItem ctl = pair.Key;
          if (initialEnabling)
          {
    ...
    private void InitializeControl(MenuItem ctl, KeyValuePair<MenuItem, CslaActionExtenderProperties> pair)
    {
    ...
    private void ChangeEnabled(MenuItem ctl, bool newEnabled)
    {
    ...
    private void CloseForm()
    {
      if (_sources.Count > 0)
      {
        Dictionary<MenuItem, CslaActionExtenderProperties>.Enumerator enumerator = _sources.GetEnumerator();
        if (enumerator.MoveNext())
        {
          MenuItem ctl = enumerator.Current.Key;
    ...
    private Form GetParentForm(MenuItem thisToolStripButton)
    {
      return GetParentForm(thisToolStripButton.GetMenuBar());
    }
