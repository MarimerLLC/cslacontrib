//-----------------------------------------------------------------------
// <copyright file="CslaActionExtenderToolBar.cs" company="Marimer LLC">
...
{
  /// <summary>
  /// Extender control (for ToolBarButton only) providing automation around
  /// data binding to CSLA .NET business objects.
  /// </summary>
  [ToolboxItem(true)]
  [ProvideProperty("ActionType", typeof (ToolBarButton))]
  [ProvideProperty("PostSaveAction", typeof (ToolBarButton))]
  [ProvideProperty("RebindAfterSave", typeof (ToolBarButton))]
  [ProvideProperty("DisableWhenClean", typeof (ToolBarButton))]
  [ProvideProperty("DisableWhenUseless", typeof (ToolBarButton))]
  [ProvideProperty("CommandName", typeof (ToolBarButton))]
  public class CslaActionExtenderToolBar : Wisej.Web.Component, IExtenderProvider
  {
    #region Constructors
    ...
    public CslaActionExtenderToolBar(IContainer container)
    {
    ...
    private Dictionary<ToolBarButton, CslaActionExtenderProperties> _sources =
      new Dictionary<ToolBarButton, CslaActionExtenderProperties>();
    ...
    {
      return extendee is ToolBarButton;
    }
    ...
    /// </summary>
    /// <param name="ctl">Reference to ToolBarButton.</param>
    /// <returns></returns>
    [Category("Csla")]
    ...
    [Browsable(false)]
    public bool GetDisableWhenClean(ToolBarButton ctl)
    {
      if (_sources.ContainsKey(ctl))
    ...
    /// <summary>
    /// Event indicating the user is clicking on the ToolBarButton.
    /// </summary>
    [Category("Csla")]
    ...
    /// <summary>
    /// Event indicating the user clicked on the ToolBarButton.
    /// </summary>
    [Category("Csla")]
    ...
    {
      ToolBarButton ctl = (ToolBarButton) sender;
    ...
    private void InitializeControls(bool initialEnabling)
    {
      ...
      List<ToolBarButton> extendedControls = new List<ToolBarButton>();
      foreach (KeyValuePair<ToolBarButton, CslaActionExtenderProperties> pair in _sources)
      {
        if (pair.Value.ActionType != CslaFormAction.None)
        {
          ToolBarButton ctl = pair.Key;
          if (initialEnabling)
          {
    ...
    private void CloseForm()
    {
      if (_sources.Count > 0)
      {
        Dictionary<ToolBarButton, CslaActionExtenderProperties>.Enumerator enumerator = _sources.GetEnumerator();
        if (enumerator.MoveNext())
        {
          ToolBarButton ctl = enumerator.Current.Key;
    ...
    private Form GetParentForm(ToolBarButton thisToolStripButton)
    {
      return GetParentForm(thisToolStripButton.Parent);
    }
